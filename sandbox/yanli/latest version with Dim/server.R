  ...
  # type of plot chosen by the user.
    output$plot <- renderPlot({
      ...
      output$Dim1 <- renderUI({
        numericInput("Dim1", "Dim 1", 1,min = 1, max = sets, step = 1)
      })
      
      output$Dim2 <- renderUI({
        numericInput("Dim2", "Dim 2", 2,min = 1, max = sets, step = 1)
      })
      ...
      if (check_mfa(mfa1) != TRUE) {
        ...
        # Depending on the user's selection, we'll creat the plot by calling the 
        # corresponding mfa plot function.
