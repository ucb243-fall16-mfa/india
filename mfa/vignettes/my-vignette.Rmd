...
#Wine data
The wine data is described in https://www.utdallas.edu/~herve/abdi-WiresCS-mfa-2013.pdf and can for example be found under https://github.com/ucb-stat243/stat243-fall-2016/tree/master/problem-sets/ﬁnal-project/data/ and will be used throughout the Vignette to provide examples.



```{r}
data <- read.csv("https://github.com/ucb-stat243/stat243-fall-2016/tree/master/problem-sets/ﬁnal-project/data/wines.csv")
...
#ctr_var_dim (Contribution of a Variable to a Dimension)
The ctr_j_l function operates on an mfa object, it returns the contribution of a variable j on a dimension l.  It takes as input an object of class mfa and the number of dimensions that will be returned. It returns a matrix of these dimensions where each entry j,l is:
...
#ctr_table_dim (Contribution of a Table to a Dimension)
The ctr_table_dim function operates on an mfa Object, it returns the contribution of a Table $k$ on a Dimension $l$. It takes as input an object of class mfa and the number of dimensions that will be returned. It returns a matrix of these dimensions where each entry is: 

$ctr_k_l = \sum_i^{J_k} ctr_j_l$ where $ctr_j_l$ is the contribution of a variable $j$ to a dimension $l$ and the sum goes over all variables in Table $k$. The contributions of a table $k$ to a dimension $l$ is therefore just the sum of the contributions of the variables comprising the table to the dimension $l$.
...
#Lg_Coefficient Function
The Lg_coefficient function ... 
